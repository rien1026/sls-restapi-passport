...
import { swaggerSpec, configSwagger } from './configs';
...
app.use((ctx: Koa.Context, next: Koa.Next) => {
	return passport.authenticate('jwt', { session: false }, async (err: any, user: any, info?: any) => {
		ctx.state['user'] = user && !user.isAdmin ? user : {};
		await next();
	})(ctx, next);
});
...
router.get('/app/login/kakao', UserController.loginSocial('kakao'));
router.get('/app/login/facebook', UserController.loginSocial('facebook'));
router.get('/app/login/naver', UserController.loginSocial('naver'));
router.get('/app/login/google', UserController.loginSocial('google'));
